// routes/api/link-lineid.js (修正後)
console.log('🧩 link-lineid HANDLER BUILD v2025-10-25-FIX', __filename); // バージョンを更新
...
router.post('/link-lineid', async (req, res) => {
  ...
  let pool;
  let tx; // トランザクション変数を try の外で宣言
  try {
    pool = await sql.connect(cfg);
    tx = new sql.Transaction(pool); // txをここで初期化
...
        ON (t.user_id = s.user_id) -- 🚨 修正点1: user_idをキーにマッチ
        WHEN MATCHED THEN
          UPDATE SET line_id = s.line_id, 更新日時 = SYSDATETIME() -- 🚨 修正点2: line_idを更新
...
      `);
    ...
    try { if (tx && tx._aborted !== true) await tx.rollback(); } catch { } // txの存在チェックを追加
