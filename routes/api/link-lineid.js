// routes/api/link-lineid.js
console.log('🧩 link-lineid HANDLER BUILD v2025-08-21-1', __filename);

const express = require('express');
const router = express.Router();
const sql = require('mssql');
const dbConfig = require('../../config/dbConfig'); // main.js と同じ
...
router.post('/link-lineid', async (req, res) => {
  ...
  let pool;
  try {
    pool = await sql.connect(cfg); // ← pool を取得してから
    const tx = new sql.Transaction(pool); // ← pool を渡してトランザクション開始
    ...
    //   b) この userId に既に別の lineId が割り当て済みか？
    ...
    if (!force) {
      if (rsLineInUse.recordset.length) {
        return res.status(409).json({
          ...
          holder: rsLineInUse.recordset[0]   // { line_id, user_id }
        });
      }
      if (rsUserHasOther.recordset.length) {
        return res.status(409).json({
          error: 'userHasOtherLine',
          current: rsUserHasOther.recordset[0] // { line_id, user_id }
        });
      }
    }
    ...
    await reqTx
      ...
      .query(`
        ...
        WHEN MATCHED THEN
          UPDATE SET user_id = s.user_id, 更新日時 = SYSDATETIME()
